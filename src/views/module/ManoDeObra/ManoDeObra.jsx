...
import {
    Eye, Edit, Trash2, Plus, ArrowLeft, Save, Users, ListFilter, XCircle, Briefcase,
...
const INITIAL_OVERALL_FORM_STATE = { dateOverallExp: dayjs().format('YYYY-MM-DD'), noveltyExpense: '', status: true, };
const INITIAL_ITEM_FORM_STATE = { idSpecificConcept: '', price: '', numEmployees: '1', addBonus: false, bonusAmount: '', };
const INITIAL_FORM_ERRORS = { dateOverallExp: null, noveltyExpense: null, expenseItems: null, idSpecificConcept: null, price: null, numEmployees: null, bonusAmount: null, };
                        ...
                        <Table hover striped size="sm" className="mb-0 custom-table align-middle">
                            <thead className="table-light">
                                <tr>
                                    ...
                                    <th scope="col" className="text-end" style={{ width: '20%' }}>Monto Total</th>
                                    <th scope="col">Novedades</th>
                                    <th scope="col" className="text-center" style={{ width: '10%' }}>Estado</th>
                                    <th scope="col" className="text-center" style={{ width: '12%' }}>Acciones</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                : currentItemsOnPage.length > 0 ? ( currentItemsOnPage.map((item) => (
                                    <tr key={item.idOverallMonth}>
                                        <th scope="row" className="text-center">{item.idOverallMonth}</th>
                                        <td>{item.dateOverallExp ? dayjs(item.dateOverallExp).format('DD/MM/YYYY') : '-'}</td>
                                        <td className="text-end fw-bold">{formatCurrencyCOP(item.valueExpense)}</td>
                                        <td className="text-truncate" style={{maxWidth: '200px'}} title={item.noveltyExpense || item.novelty_expense}>{item.noveltyExpense || item.novelty_expense || '-'}</td>
                                        <td className="text-center">
                                            <Button size="sm" className={`status-button ${item.status ? 'status-active' : 'status-inactive'}`} onClick={() => requestChangeStatusConfirmation(item)} disabled={isConfirmActionLoading || confirmModalProps.isConfirming} title={item.status ? "Activo (Click para desactivar)" : "Inactivo (Click para activar)"}>
                ...
                <>
                    ...
                    <Form id="createMonthlyExpenseForm" noValidate onSubmit={(e) => e.preventDefault()}>
                        ...
                        <div style={formSectionStyle}>
                            <h5 style={formSectionTitleStyle}>Datos Generales del Mes</h5>
                            <Row className="g-3"><Col md={6}><FormGroup><Label for="dateOverallExp" className="form-label fw-bold">Fecha Mes <span className="text-danger">*</span></Label><Input id="dateOverallExp" bsSize="sm" type="date" name="dateOverallExp" value={overallForm.dateOverallExp} onChange={handleOverallFormChange} max={dayjs().format('YYYY-MM-DD')} invalid={!!formErrors.dateOverallExp} disabled={isSubmitting} required /><FormFeedback>{formErrors.dateOverallExp}</FormFeedback></FormGroup></Col><Col md={6}><FormGroup><Label for="noveltyExpense" className="form-label fw-bold">Novedades Mes <span className="text-danger">*</span></Label><Input id="noveltyExpense" bsSize="sm" type="text" name="noveltyExpense" value={overallForm.noveltyExpense} onChange={handleOverallFormChange} placeholder="Describa eventos..." invalid={!!formErrors.noveltyExpense} disabled={isSubmitting} required /><FormFeedback>{formErrors.noveltyExpense}</FormFeedback></FormGroup></Col></Row>
                        </div>
                        <div style={formSectionStyle}>
                            <h5 style={formSectionTitleStyle}>Conceptos de Gasto</h5>
                            <div id="addExpenseItemSection" className="p-3 border rounded mb-4 bg-light">
                                <div className="d-flex justify-content-between align-items-center mb-3"><h6 className="mb-0 text-secondary">{editingItemIndex !== null ? 'Editando Concepto' : 'Agregar Nuevo Concepto'}</h6><Button color="info" outline size="sm" onClick={navigateToManageSpecificConcepts} title="Gestionar Conceptos Específicos"><SlidersHorizontal size={16} className="me-1"/> Config. Conceptos</Button></div>
                                ...
                                <Row className="g-3 mb-3 align-items-end"><Col md={isSpecialSalaryConceptSelected ? 12 : 7} lg={isSpecialSalaryConceptSelected ? 12 : (itemForm.idSpecificConcept ? 5 : 9 )}><FormGroup className="mb-md-0"><Label for="idSpecificConcept" className="form-label">Concepto de Gasto <span className="text-danger">*</span></Label><Input type="select" bsSize="sm" name="idSpecificConcept" id="idSpecificConcept" value={itemForm.idSpecificConcept} onChange={handleItemFormChange} invalid={!!formErrors.idSpecificConcept} disabled={isSubmitting || isLoadingSpecificConcepts || specificConcepts.length === 0}><option value="">{isLoadingSpecificConcepts ? "Cargando..." : specificConcepts.length === 0 ? "No hay conceptos" : "Seleccione..."}</option>{specificConcepts.map(concept => (<option key={concept.idSpecificConcept} value={concept.idSpecificConcept}>{concept.name} ({concept.expenseCategory?.name || 'Sin Cat.'})</option>))}</Input><FormFeedback>{formErrors.idSpecificConcept}</FormFeedback>{isLoadingSpecificConcepts && <Spinner size="sm" className="ms-2 mt-1"/>}</FormGroup></Col>
                                    {itemForm.idSpecificConcept && (<><Col md={isSpecialSalaryConceptSelected ? 6 : 5} lg={isSpecialSalaryConceptSelected ? 3 : 4}><FormGroup className="mb-md-0"><Label for="price" className="form-label">{isSpecialSalaryConceptSelected ? "Sueldo Base" : "Valor"} <span className="text-danger">*</span></Label><InputGroup size="sm"><InputGroupText><DollarSign size={16}/></InputGroupText><Input id="price" type="number" name="price" value={itemForm.price} onChange={handleItemFormChange} placeholder="0" min="0.01" step="any" invalid={!!formErrors.price} disabled={isSubmitting || !itemForm.idSpecificConcept} required /></InputGroup><FormFeedback className={formErrors.price ? 'd-block' : ''}>{formErrors.price}</FormFeedback></FormGroup></Col>
                                    {isSpecialSalaryConceptSelected && (<><Col md={6} lg={2}><FormGroup  className="mb-md-0"><Label for="numEmployees" className="form-label">Nº Empleados <span className="text-danger">*</span></Label><InputGroup size="sm"><InputGroupText><Hash size={16}/></InputGroupText><Input id="numEmployees" type="number" name="numEmployees" value={itemForm.numEmployees} onChange={handleItemFormChange} placeholder="1" min="1" step="1" invalid={!!formErrors.numEmployees} disabled={isSubmitting || !itemForm.idSpecificConcept} required /></InputGroup><FormFeedback className={formErrors.numEmployees ? 'd-block' : ''}>{formErrors.numEmployees}</FormFeedback></FormGroup></Col><Col md={6} lg={3}><FormGroup check inline className="mb-2 mt-md-4 pt-md-2"><Input type="checkbox" name="addBonus" id="addBonus" checked={itemForm.addBonus} onChange={handleItemFormChange} disabled={isSubmitting || !itemForm.idSpecificConcept} /><Label for="addBonus" check className="form-label">¿Bonificación?</Label></FormGroup>{itemForm.addBonus && (<FormGroup className="mb-md-0"><InputGroup size="sm"><InputGroupText><DollarSign size={16}/></InputGroupText><Input id="bonusAmount" type="number" name="bonusAmount" value={itemForm.bonusAmount} onChange={handleItemFormChange} placeholder="Monto Bonificación" min="0" step="any" invalid={!!formErrors.bonusAmount} disabled={isSubmitting || !itemForm.idSpecificConcept} /></InputGroup><FormFeedback className={formErrors.bonusAmount ? 'd-block' : ''}>{formErrors.bonusAmount}</FormFeedback></FormGroup>)}</Col><Col md={6} lg={4}><FormGroup  className="mb-md-0"><Label className="form-label fw-bold">Total Concepto:</Label><Input type="text" value={formatCurrencyCOP(calculateItemFinalPrice(itemForm))} disabled readOnly className="fw-bold text-success border-success"/></FormGroup></Col></>)}</>)}
                                    <Col md={12} lg={itemForm.idSpecificConcept ? (isSpecialSalaryConceptSelected ? 12 : 3) : 3} className="mt-3 mt-lg-0"><Button color={editingItemIndex !== null ? "warning" : "success"} onClick={addOrUpdateExpenseToTable} className="w-100" disabled={ isSubmitting || !itemForm.idSpecificConcept || !itemForm.price }>{editingItemIndex !== null ? <><Edit3 size={16} className="me-1"/> Actualizar</> : <><Plus size={16} className="me-1"/> Agregar</>}</Button>{editingItemIndex !== null && (<Button color="secondary" outline className="w-100 mt-1" onClick={resetItemForm} disabled={isSubmitting}>Cancelar Edición</Button>)}</Col>
                                </Row>
                            </div>
                            {addedExpenses.length > 0 && (<><h6 className="mt-4 mb-3">Conceptos Agregados:</h6><div className="table-responsive mb-3 shadow-sm"><Table hover striped bordered size="sm" className="align-middle custom-detail-table"><thead className="table-light"><tr><th style={{ width: '40%' }}>Concepto</th><th style={{ width: '25%' }}>Categoría</th><th style={{ width: '20%' }} className="text-end">Precio Final</th><th style={{ width: '15%' }} className="text-center">Acciones</th></tr></thead><tbody>{addedExpenses.map((item, index) => (<tr key={index} className={editingItemIndex === index ? 'table-warning' : ''}><td>{item.conceptName}{item.requiresEmployeeCalculation && (<FormText color="muted" className="d-block small">Base: {formatCurrencyCOP(item.baseSalary)} x {item.numEmployees} emp.{item.addBonus && parseFloat(item.bonusAmount) > 0 ? ` + Bono: ${formatCurrencyCOP(item.bonusAmount)}` : ''}</FormText>)}</td><td>{item.categoryName || <span className="text-muted fst-italic">N/A</span>}</td><td className="text-end fw-medium">{formatCurrencyCOP(item.price)}</td><td className="text-center"><Button color="warning" outline size="sm" className="p-1 me-1 btn-icon-only" onClick={() => startEditAddedItem(index)} disabled={isSubmitting} title="Editar"><Edit3 size={14} /></Button><Button color="danger" outline size="sm" className="p-1 btn-icon-only" onClick={() => removeExpenseFromTable(index)} disabled={isSubmitting} title="Quitar"><Trash2 size={14} /></Button></td></tr>))}</tbody><tfoot className="table-light"><tr><th colSpan="2" className="text-end fw-bold">Total Agregado:</th><th className="text-end fw-bold">{formatCurrencyCOP(totalExpensesInForm)}</th><th></th></tr></tfoot></Table></div></>)}
