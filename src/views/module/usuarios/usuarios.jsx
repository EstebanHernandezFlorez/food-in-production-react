...
const INITIAL_FORM_STATE = { idUsers: "", document_type: "", document: "", cellphone: "", full_name: "", email: "", idRole: "", password: "", confirmPassword: "", status: "true" };
...
const Usuario = () => {
    ...
    const validateFullForm = useCallback(() => { /* ... */ let errors = { ...INITIAL_FORM_ERRORS }; let isValid = true; let firstErrorMessage = ''; for (const key in INITIAL_FORM_STATE) { if (key === 'idUsers') continue; if (isEditing && (key === 'password' || key === 'confirmPassword') && !form.password && !form.confirmPassword) continue; const error = validateField(key, form[key], form, isEditing); if (error) { errors[key] = error; isValid = false; if (!firstErrorMessage) firstErrorMessage = error; } } if (initialDataLoaded && isValid) { const emailLower = form.email.trim().toLowerCase(); const documentLower = form.document.trim().toLowerCase(); const duplicate = data.find(u => { const isDifferentUser = isEditing ? u.idUsers !== form.idUsers : true; return isDifferentUser && ((u.email && u.email.toLowerCase() === emailLower) || (u.document && String(u.document).toLowerCase() === documentLower)); }); if (duplicate) { isValid = false; const field = duplicate.email.toLowerCase() === emailLower ? 'email' : 'document'; errors[field] = `Este ${field === 'email' ? 'correo' : 'documento'} ya está registrado.`; if (!firstErrorMessage) firstErrorMessage = errors[field]; } } if (!isValid && !errors.general) errors.general = firstErrorMessage || "Por favor, corrija los errores."; setFormErrors(errors); return isValid; }, [form, data, isEditing, initialDataLoaded]);
    ...
    const openEditModal = useCallback((user) => { setIsEditing(true); resetFormAndErrors(); setForm({ idUsers: user.idUsers || "", document_type: user.document_type || "", document: String(user.document || ""), cellphone: String(user.cellphone || ""), full_name: user.full_name || "", email: user.email || "", idRole: user.idRole !== undefined ? String(user.idRole) : "", password: "", confirmPassword: "", status: user.status !== undefined ? (user.status ? "true" : "false") : "true", }); setModalOpen(true); }, [resetFormAndErrors]);
    ...
    // --- Memos de Paginación y Filtrado (sin cambios) ---
    ...
    // <-- CAMBIO: Se añade un memo para ordenar los datos, igual que en Insumos.
    const sortedItems = useMemo(() => {
        return [...currentItems].sort((a, b) => (a.idUsers || 0) - (b.idUsers || 0));
    }, [currentItems]);

    // --- Acciones CRUD (lógica sin cambios) ---
    const handleFormSubmit = async () => { /* ... */ if (!validateFullForm()) { toast.error(formErrors.general || "Por favor, revise los campos marcados."); return; } setIsSubmitting(true); const toastId = toast.loading(isEditing ? 'Actualizando usuario...' : 'Agregando usuario...'); const { idUsers, confirmPassword, ...formData } = form; const payload = { ...formData, idRole: parseInt(formData.idRole, 10), status: formData.status === "true", }; if (!isEditing) payload.password = formData.password; else if (isEditing && formData.password) payload.password = formData.password; else delete payload.password; try { if (isEditing) await userService.updateUser(idUsers, payload); else await userService.createUser(payload); toast.success(isEditing ? 'Usuario actualizado!' : 'Usuario agregado!', { id: toastId }); toggleMainModal(); await refreshData(false); setCurrentPage(isEditing ? validCurrentPage : 1); } catch (error) { const errorData = error.response?.data; let serverErrorMsg = "Error desconocido."; const newFormErrors = { ...INITIAL_FORM_ERRORS, general: '' }; if (errorData?.errors && Array.isArray(errorData.errors)) { serverErrorMsg = errorData.errors[0]?.msg || "Error de validación del servidor."; errorData.errors.forEach(err => { if (err.path && !newFormErrors[err.path]) newFormErrors[err.path] = err.msg; }); } else if (errorData?.message) { serverErrorMsg = errorData.message; if (serverErrorMsg.toLowerCase().includes('email') && (serverErrorMsg.toLowerCase().includes('duplicate') || serverErrorMsg.toLowerCase().includes('ya existe'))) newFormErrors.email = "Este correo ya está registrado."; else if (serverErrorMsg.toLowerCase().includes('document') && (serverErrorMsg.toLowerCase().includes('duplicate') || serverErrorMsg.toLowerCase().includes('ya existe'))) newFormErrors.document = "Este documento ya está registrado."; } else if (error.message) serverErrorMsg = error.message; newFormErrors.general = serverErrorMsg; setFormErrors(newFormErrors); toast.error(`Error: ${newFormErrors.general || serverErrorMsg}`, { id: toastId, duration: 5000 }); } finally { setIsSubmitting(false); } };
    const executeChangeStatus = useCallback(async (user) => { /* ... */ const { idUsers, status: currentStatus, full_name } = user; const newStatus = !currentStatus; const actionText = newStatus ? "activado" : "desactivado"; const toastId = toast.loading(`${newStatus ? 'Activando' : 'Desactivando'} "${full_name || ''}"...`); try { await userService.changeStateUser(idUsers, newStatus); toast.success(`Usuario "${full_name || ''}" ${actionText}.`, { id: toastId }); await refreshData(false); } catch (error) { toast.error(`Error: ${error.response?.data?.message || error.message}`, { id: toastId }); throw error; } }, [refreshData]);
    ...
    const executeDelete = useCallback(async (user) => { /* ... */ const { idUsers, full_name } = user; const toastId = toast.loading(`Eliminando "${full_name || ''}"...`); try { await userService.deleteUser(idUsers); toast.success(`Usuario "${full_name || ''}" eliminado.`, { id: toastId }); const newTotalItemsAfterDelete = totalItems - 1; const newTotalPagesAfterDelete = Math.ceil(newTotalItemsAfterDelete / ITEMS_PER_PAGE) || 1; if (currentItems.length === 1 && currentPage > 1 && newTotalItemsAfterDelete > 0) { setCurrentPage(prev => Math.max(1, prev - 1)); } else if (currentPage > newTotalPagesAfterDelete) { setCurrentPage(newTotalPagesAfterDelete); } await refreshData(false); } catch (error) { toast.error(`Error: ${error.response?.data?.message || error.message}`, { id: toastId }); throw error; } }, [refreshData, currentPage, currentItems, totalItems]);
    ...
    return (
        <Container fluid className="p-4 main-content">
            ...
            {/* <-- CAMBIO: Se aplica el estilo a la tabla y su contenedor --> */}
            <div className="table-responsive shadow-sm custom-table-container mb-3">
                 <Table hover striped size="sm" className="mb-0 custom-table align-middle">
                     {/* <-- CAMBIO: `thead` con `table-light` y `th` con `scope` y anchos definidos --> */}
                    ...
                    <tbody>
                         ...
                         // <-- CAMBIO: Se itera sobre `sortedItems` en lugar de `currentItems` -->
                            ...
                            sortedItems.map((item) => (
                                <tr key={item.idUsers}>
                                    <th scope="row" className="text-center">{item.idUsers}</th>
                                    ...
                                    <td className="text-center">
                                        <div className="d-inline-flex gap-1">
                                            {/* <-- CAMBIO: Botones de acción con estilo `outline` --> */}
