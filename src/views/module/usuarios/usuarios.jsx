...
// ... (ConfirmationModal, Constantes y validateField sin cambios)
...
const Usuario = () => {
    // ... (Todos los estados y hooks iniciales se mantienen igual)
    ...
    const validateFullForm = useCallback(() => { let errors = { ...INITIAL_FORM_ERRORS }; let isValid = true; let firstErrorMessage = ''; for (const key in INITIAL_FORM_STATE) { if (key === 'idUser') continue; if (isEditing && (key === 'password' || key === 'confirmPassword') && !form.password && !form.confirmPassword) continue; const error = validateField(key, form[key], form, isEditing); if (error) { errors[key] = error; isValid = false; if (!firstErrorMessage) firstErrorMessage = error; } } if (!isValid) { setFormErrors(errors); return false; } if (initialDataLoaded) { const emailLower = form.email.trim().toLowerCase(); const documentLower = form.document.trim().toLowerCase(); const duplicateEmailUser = data.find(user => user.email.toLowerCase() === emailLower && (!isEditing || user.idUser != form.idUser)); if (duplicateEmailUser) { isValid = false; errors.email = "Este correo electrónico ya está registrado."; if (!firstErrorMessage) firstErrorMessage = errors.email; } const duplicateDocumentUser = data.find(user => String(user.document).toLowerCase() === documentLower && (!isEditing || user.idUser != form.idUser)); if (duplicateDocumentUser) { isValid = false; errors.document = "Este documento ya está registrado."; if (!firstErrorMessage) firstErrorMessage = errors.document; } } if (!isValid && !errors.general) { errors.general = firstErrorMessage || "Por favor, corrija los errores."; } setFormErrors(errors); return isValid; }, [form, data, isEditing, initialDataLoaded]);
    ...
    const openEditModal = useCallback((user) => { setIsEditing(true); resetFormAndErrors(); const userFormState = { idUser: user.idUser || "", document_type: user.document_type || "", document: String(user.document || ""), cellphone: String(user.cellphone || ""), full_name: user.full_name || "", email: user.email || "", idRole: user.idRole !== undefined ? String(user.idRole) : "", password: "", confirmPassword: "", status: user.status !== undefined ? String(user.status) : "true", }; setForm(userFormState); setOriginalForm(userFormState); setModalOpen(true); }, [resetFormAndErrors]);
    ...
    return (
        <Container fluid className="p-4 main-content">
            {/* === INICIO CAMBIOS: ESTILOS PARA LA TABLA === */}
            <style>
                {`
                .custom-table {
                    table-layout: fixed; /* Evita que el contenido expanda las columnas */
            ...
            <div className="table-responsive shadow-sm custom-table-container mb-3">
                 <Table hover striped size="sm" className="mb-0 custom-table align-middle">
                     <thead className="table-light">
                        <tr>
                            {/* === INICIO CAMBIOS: AJUSTE DE ANCHOS DE COLUMNA === */}
                            <th scope="col" style={{ width: '5%' }}>ID</th>
                            <th scope="col" style={{ minWidth: '150px' }}>Nombre Completo</th>
                            <th scope="col" style={{ width: '8%' }}>Tipo</th>
                            <th scope="col" style={{ width: '12%' }}>Documento</th>
                            <th scope="col" style={{ minWidth: '180px' }}>Correo</th>
                            <th scope="col" style={{ width: '11%' }}>Celular</th>
                            <th scope="col" style={{ width: '10%' }}>Rol</th>
                            <th scope="col" className="text-center" style={{ width: '8%' }}>Estado</th>
                    ...
                    <tbody>
                         {isLoading && !initialDataLoaded ? (
                            ...
                            sortedItems.map((item) => (
                                <tr key={item.idUser}>
                                    <td className="text-center fw-bold">{item.idUser}</td>
                ...
                <ModalBody>
                    ...
                    <Form id="userForm" noValidate onSubmit={(e) => {e.preventDefault(); handleFormSubmit();}}>
                        ...
                        <Row className="g-3">
                            ...
                            {!isEditing && (
                                <>
                                    <Col md={6}><FormGroup><Label for="password_modal_add">Contraseña <span className="text-danger">*</span></Label><Input id="password_modal_add" name="password" type="password" value={form.password} onChange={handleInputChange} invalid={!!formErrors.password} autoComplete="new-password" /><FormFeedback>{formErrors.password}</FormFeedback></FormGroup></Col>
                                    <Col md={6}><FormGroup><Label for="confirmPassword_modal_add">Confirmar Contraseña <span className="text-danger">*</span></Label><Input id="confirmPassword_modal_add" name="confirmPassword" type="password" value={form.confirmPassword} onChange={handleInputChange} invalid={!!formErrors.confirmPassword} autoComplete="new-password" /><FormFeedback>{formErrors.confirmPassword}</FormFeedback></FormGroup></Col>
                                    <Col xs={12}><small className="text-muted d-block mt-n2">10 caracteres, con mayúscula, minúscula, número y símbolo (@$!%*?&).</small></Col>
                                </>
                            )}
                             {isEditing && (
                                <>
                                    <Col xs={12} className="mt-3"><hr/><h6 className="mb-3 text-muted">Cambiar Contraseña (opcional)</h6></Col>
                                    <Col md={6}><FormGroup><Label for="password_modal_edit">Nueva Contraseña</Label><Input id="password_modal_edit" name="password" type="password" placeholder="Dejar en blanco para no cambiar" value={form.password} onChange={handleInputChange} invalid={!!formErrors.password && !!form.password} autoComplete="new-password" />{form.password && <FormFeedback>{formErrors.password}</FormFeedback>}</FormGroup></Col>
                                    <Col md={6}><FormGroup><Label for="confirmPassword_modal_edit">Confirmar Nueva Contraseña</Label><Input id="confirmPassword_modal_edit" name="confirmPassword" type="password" placeholder="Dejar en blanco para no cambiar" value={form.confirmPassword} onChange={handleInputChange} invalid={!!formErrors.confirmPassword && !!form.password} autoComplete="new-password" />{form.password && <FormFeedback>{formErrors.confirmPassword}</FormFeedback>}</FormGroup></Col>
                                    {(form.password || form.confirmPassword) && <Col xs={12}><small className="text-muted d-block mt-n2">10 caracteres, con mayúscula, minúscula, número y símbolo (@$!%*?&).</small></Col>}
                                </>
                            )}
                            {/* === FIN CAMBIOS === */}
                        </Row>
